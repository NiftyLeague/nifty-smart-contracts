...
describe('BalanceTree', () => {
  ...
  it('rejects a tampered proof element', () => {
    ...
    proof = proof.map((p, i) =>
      i === 0 ? Buffer.concat([p.subarray(0, -4), Buffer.from('abcd', 'hex')]) : p
    )
  ...
  it('rejects a proof that does not include the claimed leaf element', () => {
    ...
    const wrongProof = proofToBuffers(
      otherTree.getProof(0, otherBalances[0].account, otherBalances[0].amount)
    )
  ...
  it('verifies with bigint index input', () => {
    ...
    expect(BalanceTree.verifyProof(0n, balances[0].account, balances[0].amount, proof, root)).toBe(
      true
    )
  })
})
