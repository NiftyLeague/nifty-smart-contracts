...
describe('BASE_DEGENS_URI', () => {
  it('includes the network name in the path', () => {
    expect(BASE_DEGENS_URI(NetworkName.Mainnet)).toBe(
      'https://api.niftyleague.com/mainnet/degen/metadata/'
    )
  })

  it('uses sepolia for sepolia network', () => {
    expect(BASE_DEGENS_URI(NetworkName.Sepolia)).toBe(
      'https://api.niftyleague.com/sepolia/degen/metadata/'
    )
  })
