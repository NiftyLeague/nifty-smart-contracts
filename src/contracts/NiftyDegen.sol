...
contract NiftyDegen is NameableCharacter {
  ...
  ) NiftyLeagueCharacter(nftlAddress, 'NiftyDegen', 'DEGEN') {
  ...
  function getNFTPrice() public view returns (uint256 price) {
    uint256 currentSupply = totalSupply.current();
    require(
      currentSupply < MAX_SUPPLY - SPECIAL_CHARACTERS ||
        (_msgSender() == owner() && currentSupply < MAX_SUPPLY),
   ...
   */
  function isAvailableAndAllowedTrait(
    uint256 tribe,
    uint256 trait
  ...
  function _validateTraits(
    ...
    require(
      _isTraitInRange(char[1], 10, 69) || _isTraitInRange(char[1], 119, 149),
      'Skin color incorrect'
    );
    require(
      _isTraitInRange(char[2], 70, 100) || _isTraitInRange(char[2], 119, 149),
      'Fur color incorrect'
    );
    require(
      _isTraitInRange(char[3], 101, 109) || _isTraitInRange(char[3], 119, 149),
      'Eye color incorrect'
    );
    require(
      _isTraitInRange(char[4], 110, 118) || _isTraitInRange(char[4], 119, 149),
      'Pupil color incorrect'
    );
  ...
  function _rngIndex(uint256 tokenId) private view returns (uint256 index) {
    uint256 randomHash = uint256(
      keccak256(abi.encodePacked(tokenId, block.timestamp, block.basefee))
    );
  ...
  function _rngSkip(uint256 poolSize) private view returns (bool skip) {
    uint256 randomHash = uint256(
      keccak256(abi.encodePacked(poolSize, block.timestamp, block.basefee))
    );
    // slither-disable-next-line weak-prng
