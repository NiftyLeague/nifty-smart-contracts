...
abstract contract NameableCharacter is NiftyLeagueCharacter {
  ...
  ) external returns (string memory name) {
    if (!_exists(tokenId)) revert NameError('nonexistent token');
    if (!_isApprovedOrOwner(_msgSender(), tokenId))
      revert NameError('Caller is not owner nor approved');
    string memory prevName = _characters[tokenId].name;
    if (sha256(bytes(newName)) == sha256(bytes(prevName)))
      revert NameError('New name and old name are equal');
