  ...
  */
  ...
  if (!localNetwork) {
    ...
    await etherscanVerify({
      ...
      constructorArguments: [emissionStartTimestamp],
    })
    await etherscanVerify({
      address: degenAddress,
      constructorArguments: [nftlTokenAddress, storageAddress],
    })
    await etherscanVerify({
...
const deploy = async (
  ...
  const deployedContract: DeployedContract = await contractFactory.deploy(
    ...contractArgs,
    overrides
  )
...
const getOrCreateContract = async (
  ...
  if (fs.existsSync(contractAddress) && !localNetwork) {
    ...
    const contract = await ethers.getContractAt(
      contractName,
      fs.readFileSync(contractAddress).toString()
    )
