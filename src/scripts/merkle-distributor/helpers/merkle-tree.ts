...
export default class MerkleTree {
  ...
  constructor(elements: Buffer[]) {
    ...
    this.bufferElementPositionIndex = this.elements.reduce<{ [hexElement: string]: number }>((memo, el, index) => {
      memo[MerkleTree.bufferToHex(el)] = index;
      return memo;
    }, {});
  ...
  static combinedHash(first: Buffer, second: Buffer): Buffer {
    ...
    return Buffer.from(
      keccak256(MerkleTree.sortAndConcat(first, second)).slice(2), // Remove '0x' and convert to Buffer
      'hex',
    );
  }
  ...
  getHexRoot(): string {
    return MerkleTree.bufferToHex(this.getRoot());
  }

  getProof(el: Buffer): Buffer[] {
    let idx = this.bufferElementPositionIndex[MerkleTree.bufferToHex(el)];
