...
import type {
  ...
  BalanceManagerDistributor__factory,
  NFTLToken,
...
describe('BalanceManagerDistributor', () => {
  ...
  describe('#admin', () => {
    it('cannot withdraw during claim', async () => {
      ...
      await expect(distributor.withdraw()).to.be.revertedWithCustomError(
        distributor,
  ...
  describe('#claim', () => {
    it('fails for empty proof', async () => {
      ...
      await expect(
        distributor.claim(0, await wallet0.getAddress(), 10n, [])
    ...
    describe('two account tree', () => {
      ...
      xit('gas', async () => {
        ...
        expect(receipt?.gasUsed).to.eq(
          gasUsed['BalanceManagerDistributor' as keyof typeof gasUsed].twoAccountTree
        )
      })
    })
  })
