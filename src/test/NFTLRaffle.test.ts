...
describe('NFTLRaffle', function () {
  ...
  describe('manageConsumers', () => {
    ...
    it('Should revert if the caller is not the owner', async () => {
      await expect(
        nftlRaffle.connect(alice).manageConsumers(await nftlRaffle.getAddress(), true)
      ).to.be.revertedWith('Ownable: caller is not the owner')
      await expect(
        nftlRaffle.connect(alice).manageConsumers(await nftlRaffle.getAddress(), false)
      ).to.be.revertedWith('Ownable: caller is not the owner')
    })
  })
  ...
  describe('distributeTicketsToUsers', () => {
    ...
    it('Shoud be able to distribute tickets to users', async () => {
      ...
      expect(await nftlRaffle.userDeposits(await john.getAddress())).to.equal(
        johnDepositBefore + nftlAmountToDeposit
      )
      expect(await nftlRaffle.userDeposits(await dan.getAddress())).to.equal(
        nftlAmountPerTicket * 50n
      )
    ...
    it('Should revert if the coller is not the owner', async () => {
      ...
      await expect(
        nftlRaffle.connect(john).distributeTicketsToUsers(users, ticketCount)
      ).to.be.revertedWith('Ownable: caller is not the owner')
    })
  })
  ...
  describe('deposit', () => {
    ...
    it('Should be able to deposit NFTL tokens', async () => {
      ...
      expect(await nftlToken.balanceOf(await alice.getAddress())).equal(
        aliceNFTLBalanceBefore - nftlAmountToDeposit
      )
      ...
      expect(await nftlToken.balanceOf(await bob.getAddress())).equal(
        bobNFTLBalanceBefore - nftlAmountToDeposit
      )
      expect(await nftlRaffle.userDeposits(await bob.getAddress())).to.equal(
        bobDepositsBefore + nftlAmountToDeposit
      )
  ...
  describe('assignTicketToUsers', () => {
    ...
    it('Should be able to assign the tickets to users', async () => {
      ...
      expect(await nftlRaffle.ticketRangeByUser(await bob.getAddress())).to.deep.equal([
        924n,
        1044n,
      ])
      expect(await nftlRaffle.ticketRangeByUser(await john.getAddress())).to.deep.equal([
        1045n,
        1466n,
      ])
  ...
  describe('requestRandomWordsForWinnerSelection & rawFulfillRandomWords', () => {
    ...
    it('Should not request the random words if it already exists', async () => {
      ...
      await nftlRaffle
        .connect(impersonatedCoordinator)
    ...
    it('Should revert if the coller is not the owner', async () => {
      await expect(
        nftlRaffle.connect(alice).requestRandomWordsForWinnerSelection()
      ).to.be.revertedWith('Ownable: caller is not the owner')
    })
