...
describe('IMX - Store', function () {
  ...
  before(async () => {
    ...
    // init Store prices
    await storeContract.listNewItems(
      [10, 11],
  ...
  describe('NiftyMarketplace', function () {
    it('Should grant minter role to Store', async function () {
      ...
      expect(await niftyMarketplace.hasRole(MINTER_ROLE, await storeContract.getAddress())).to.be
        .true
    })
  })
  ...
  describe('ERC20 permit', function () {
    ...
    it('Should revert if permit is not used', async function () {
      ...
      await expect(storeContract.purchaseItems([10], [1])).to.be.revertedWith(
    ...
    it('Should revert with expired permit', async function () {
      ...
      await expect(
        storeContract.purchaseItemsWithPermit([10], [1], value, deadline, { v, r, s })
  ...
  describe('Store', function () {
    ...
    describe('Functionality', function () {
      ...
      it('Should transfer the NFTL to treasury', async function () {
        ...
        const deadline = ethers.MaxUint256
        const { v, r, s } = await getERC20Permit(
          bob,
          nftlToken,
          storeContract,
          expectedValue,
        ...
        await expect(
          storeContract.purchaseItemsForWithPermit(bob, items, values, expectedValue, deadline, {
            v,
            r,
            s,
          })
        ...
        expect(await nftlToken.balanceOf(NIFTY_LEDGER_DEPLOYER)).to.equal(
          treasuryInitialBal + expectedValue * 2n
        )
      })
    })
  })
