...
describe('BalanceManager', function () {
  ...
  describe('deposit', function () {
    ...
    it('should track multiple deposits', async () => {
      ...
      expect(await nftl.balanceOf(bobAddress)).to.equal(
        INIT_BALANCE - DEPOSIT_AMOUNT - DEPOSIT_AMOUNT
      )
    })
  })
  ...
  describe('withdraw', function () {
    ...
    it('should be able to withdraw NFTL tokens', async () => {
      ...
      await balanceManager
        .connect(alice)
        .withdraw(WITHDRAW_AMOUNT, nonceForAlice, expireAtForAlice, signatureForAlice)

      expect(await nftl.balanceOf(aliceAddress)).to.equal(
        INIT_BALANCE - DEPOSIT_AMOUNT + WITHDRAW_AMOUNT
      )
    })
    ...
    it('revert if the amount is wrong', async () => {
      ...
      await expect(
        balanceManager
          .connect(alice)
          .withdraw(WITHDRAW_AMOUNT + 1, nonceForAlice, expireAtForAlice, signatureForAlice)
      ).to.be.revertedWithCustomError(balanceManager, 'SignError')
    })
  })
  ...
  describe('withdrawByDAO', function () {
    ...
    it('should be able to withdraw NFTL tokens', async () => {
      ...
      expect(await nftl.balanceOf(balanceManagerAddress)).to.equal(
        2 * DEPOSIT_AMOUNT - WITHDRAW_AMOUNT
      )
