...
describe('IMX - Comics Burning', function () {
  ...
  describe('NiftyMarketplace', function () {
    it('Should grant minter role to ComicsBurner', async function () {
      ...
      expect(await niftyMarketplace.hasRole(MINTER_ROLE, await comicsBurner.getAddress())).to.be
        .true
    })
  ...
  describe('ERC1155 permit', function () {
    ...
    it('Should revert with expired permit', async function () {
      ...
      await expect(
        comicsBurner.burnComicsWithPermit([1], [3], deadline, signature)
      ).to.be.revertedWithCustomError(niftyMarketplace, 'PermitExpired')
    })
  ...
  describe('ComicsBurner', function () {
    ...
    it('Should revert with excess burn values', async function () {
      await expect(comicsBurner.burnComics([1], [15])).to.be.revertedWith(
        'ERC1155: burn amount exceeds totalSupply'
      )
      await expect(comicsBurner.burnComics([1], [8])).to.be.revertedWith(
    ...
    it('Should burn remaining comics with permit', async function () {
      ...
      const { signature } = await getERC1155Permit(signer, niftyMarketplace, comicsBurner, deadline)
      await expect(
        comicsBurner.burnComicsWithPermit(
          [1, 2, 3, 4, 5, 6],
          [2, 2, 1, 1, 0, 0],
          deadline,
          signature
    ...
    it('Should be able to burn on behalf of owner given signature', async function () {
      ...
      const { signature } = await getERC1155Permit(bob, niftyMarketplace, comicsBurner, deadline)
      await expect(
        comicsBurner.burnComicsForWithPermit(
          bob,
          [1, 2, 3, 4, 5, 6],
          [4, 4, 4, 4, 4, 4],
          deadline,
