...
describe('NiftySale', function () {
  ...
  describe('updateTokenPercentages', () => {
    it('Should be able to update the percentages', async () => {
      ...
      // update percentages
      await itemSale.updateTokenPercentages(
        newBurnPercentage,
        newTreasuryPercentage,
        newDAOPercentage
      )
  ...
  describe('withdraw', () => {
    ...
    it('Should withdraw NFTL tokens', async () => {
      ...
      // check the balances
      expect(await nftl.totalSupply()).to.equal(
        totalSupply - (totalPrice * BURN_PERCENTAGE) / 1000n
      )
      expect(await nftl.balanceOf(await treasury.getAddress())).to.equal(
        (totalPrice * TREASURY_PERCENTAGE) / 1000n
      )
      expect(await nftl.balanceOf(await dao.getAddress())).to.equal(
        (totalPrice * DAO_PERCENTAGE) / 1000n
      )
    })
